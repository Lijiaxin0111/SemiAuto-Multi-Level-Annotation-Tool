...
class PressVOID_Interaction:
    def __init__(
        self,
        prev_mask: torch.Tensor,
        prev_void_mask: torch.Tensor,
        true_size: Tuple[int, int],
        radius: int,
    ):

        self.prev_void_mask = prev_void_mask
        self.prev_mask = prev_mask

        self.h, self.w = true_size
        self.tar_obj = None

        self.out_void_mask = None
        self.r = radius

    def predict(self, x: int, y: int, is_neg: bool) -> torch.Tensor:
        last_void_mask = self.prev_void_mask

        circular_mask = self.create_circular_mask(
            self.h, self.w, center=(x, y), radius=self.r
        )


        last_void_mask[circular_mask] = 1 if (not is_neg) else 0
        void_mask = last_void_mask

        return void_mask, self.prev_mask
...
class PressInteraction:
    ...
    def predict(self, x: int, y: int, is_neg: bool) -> torch.Tensor:
        ...
        last_obj_mask[0, 0, circular_mask] = 0.9 if (not is_neg) else 0.1
        obj_mask = last_obj_mask



        self.out_prob = self.prev_mask.clone()
        self.out_prob[self.tar_obj] = obj_mask
        self.out_prob = aggregate_wbg(self.out_prob[1:], keep_bg=True, hard=True)
        return self.out_prob
