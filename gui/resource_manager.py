...
class ResourceManager:
    def __init__(self, cfg: DictConfig):
        ...
        self.void_mask_dir = path.join(self.workspace, "void_masks")

        os.makedirs(self.image_dir, exist_ok=True)
        os.makedirs(self.mask_dir, exist_ok=True)
        os.makedirs(self.visualization_dir, exist_ok=True)
        os.makedirs(self.soft_mask_dir, exist_ok=True)
        os.makedirs(self.void_mask_dir, exist_ok=True)
        ...
        self.get_void_mask = LRU(
            self._get_void_mask_unbuffered, maxsize=cfg["buffer_size"]
        )

        if "extract_fps" in cfg:
        ...
        if need_decoding:
            self._extract_frames(video, fps=extract_fps)
    ...
    def save_thread(self, queue: Queue):
        while True:
            ...
            elif args.type == "void_mask":
                # PIL image
                args.data.save(path.join(self.void_mask_dir, args.name + ".png"))
    ...
    def _extract_frames(self, video, fps=0):
        ...
        print(f"Extracting frames from {video} into {self.image_dir} at {fps} fps...")
        with tqdm() as bar:
            while cap.isOpened():
                ret, frame = cap.read()
                if not ret:
                ...
                if frame_index % frame_interval == 0:
                    ...
                    cv2.imwrite(
                        path.join(
                            self.image_dir, f"{frame_index // frame_interval:07d}.jpg"
                        ),
                        frame,
                    )
                    bar.update()
    ...
    def save_void_mask(self, ti: int, void_mask: np.ndarray):
        # mask should be uint8 H*W without channels
        assert 0 <= ti < self.length
        assert isinstance(void_mask, np.ndarray)

        void_mask = Image.fromarray(void_mask)
        void_mask.putpalette(self.palette)
        self.invalidate(ti)
        self.add_to_queue_with_warning(SaveItem("void_mask", void_mask, self.names[ti]))
    ...
    def _get_void_mask_unbuffered(self, ti: int):
        # returns H*W uint8 array
        assert 0 <= ti < self.length

        mask_path = path.join(self.void_mask_dir, self.names[ti] + ".png")
        if path.exists(mask_path):
            mask = Image.open(mask_path)
            mask = np.array(mask)
            return mask
