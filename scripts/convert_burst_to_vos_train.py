...
def process_video(sequence):
    ...
    for segmentation, image_path in zip(segmentations, annotated_image_paths):
        ...
        copy2(
            path.join(frames_path, dataset, seq_name, image_path), output_image_folder
        )

    if args.save_all_image:
        output_all_image_folder = path.join(
            output_path, "JPEGImages_all_frames", new_seq_name
        )
        os.makedirs(output_all_image_folder, exist_ok=True)
        all_image_paths = sequence["all_image_paths"]
        for image_path in all_image_paths:
            copy2(
                path.join(frames_path, dataset, seq_name, image_path),
                output_all_image_folder,
            )
